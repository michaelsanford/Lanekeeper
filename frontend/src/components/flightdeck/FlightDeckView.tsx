    ...
    <div className="flex-1 overflow-y-auto p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between bg-gradient-to-r from-slate-900 to-indigo-950/40 p-5 rounded-2xl border border-indigo-900/40 shadow-lg">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-all shadow-md shadow-indigo-600/20 active:scale-95"
        >
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="space-y-3">
            ...
            {inFlightTasks.length === 0 ? (
              <div className="p-8 border border-dashed border-slate-800 rounded-2xl text-center space-y-3 bg-slate-900/40">
                ...
                <button
                  onClick={onOpenQuickCapture}
                  className="inline-flex items-center gap-2 text-sm bg-slate-800 hover:bg-slate-700 text-slate-200 px-3.5 py-2 rounded-lg border border-slate-700 transition-colors"
                >
                  <Plus className="w-4 h-4" />
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-sm font-bold text-indigo-400 bg-indigo-950/80 px-2 py-0.5 rounded border border-indigo-800/60">
                            {task.key}
                          </span>
          ...
          <div className="space-y-3">
            ...
            {dueTodayOrOverdueTasks.length === 0 ? (
              <p className="text-sm text-slate-400 italic p-3 bg-slate-900/30 rounded-xl border border-slate-800/50">
                ...
                {dueTodayOrOverdueTasks.map((task) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="font-mono text-sm font-medium text-amber-400">{task.key}</span>
        ...
        <div className="space-y-3">
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-3 flex flex-col h-[22rem]">
            <textarea
              ...
              className="w-full flex-1 bg-transparent text-sm text-slate-200 outline-none resize-none placeholder-slate-500 font-mono leading-relaxed"
            />
